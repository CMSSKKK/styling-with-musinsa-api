...
public interface ItemRepository extends JpaRepository<Item, Long> {
    ...
    @Query("select i from Item i " +
            ...
            "and i.price = (select min(it.price) from Item it)")
    Optional<Item> findMinByCategory(@Param("category_id") Long categoryId);

    @Query("select i from Item i " +
            "join fetch Category c " +
            "join fetch Brand b " +
            "where i.category.id = :category_id " +
            "and i.price = (select max(it.price) from Item it)")
    Optional<Item> findMaxByCategory(@Param("category_id") Long categoryId);
